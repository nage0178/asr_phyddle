...
#if len(sys.argv) == 6:
remove_big_files = False  #sys.argv[5] == "True"
...
#    'stop_time'          : None,            # time to stop simulation 
    ...
    'max_num_taxa'       : 50,             # max number of taxa for valid sim
    'max_num_unsampled_lineages' : 10000,      # max_num_taxa * max_num_unsampled_lineages == stopping condition
    'prop_extant_sampled' : 0.0001,           # Expected proportion of lineages at stop_time to be sampled in tree
    'num_sample_time_pts' : 50,          # number of evenly spaced tim pts to sample population sizes
    'rv_fn'              : {                # distributions for model params
        ...
        'nSampled_tips'       : sp.stats.uniform.rvs,
    ...
    'rv_arg'                : {                # loc/scale/shape for param dists
        ...
        'Sample'            : { 'loc' :  10**-1,   'scale' : 1   }, # 10 to 1000 days, rate of 0.001 to 1
        'Migrate'           : { 'loc' : 10**-5, 'scale' : 4.9 * 10**-3 },
        'S0'                : { 'loc' : 10**4, 'scale' : 0  }, # 1000 to 10000 ind. in population
        'Stop_time'         : { 'loc' : 50,      'scale' : 0    },  # between 10 days and 2 months
        'nSampled_tips'     : { 'loc' : 50.0,  'scale' : 0.   },   # subsample samples
...
for idx in range(start_idx, start_idx + batch_size) : 
    ...
    if (os.path.exists(dat_nex_fn)):
        print("skipping " + phy_nex_fn)
        continue
    ...
    # call BEAST
    x = subprocess.run(['/home/anagel/beast/bin/beast', '-threads', '1', xml_fn], capture_output=True)
    #x = subprocess.run(['/home/anagel/beast/bin/beast', xml_fn], capture_output=True)
    ...
    #if num_tips < 100 or my_model.params['Stop_time'][0] != sim_stats['actual_sim_time'][0] or most_recent_tip_age != 0:
    while num_tips < 50: # or my_model.params['Stop_time'][0] != sim_stats['actual_sim_time'][0] or most_recent_tip_age != 0:
    #while num_tips < 100 or my_model.params['Stop_time'][0] != sim_stats['actual_sim_time'][0] or most_recent_tip_age != 0:
