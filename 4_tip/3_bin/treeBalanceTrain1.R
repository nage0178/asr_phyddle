...
est <- read.csv("estimate_first_train/out.empirical_est.labels_cat.csv")
...
H5Fclose(H5_truth)
print("starting one")
est <- read.csv("../one/accuracy/estimate/out.empirical_est.labels_cat.csv")
...
est <- read.csv("../8_cat/accuracy_est/out.empirical_est.labels_cat.csv")
...
write.csv(symmMat, "sym_train1.csv", row.names = FALSE)
write.csv(asymmMat, "asym_train1.csv", row.names = FALSE)
